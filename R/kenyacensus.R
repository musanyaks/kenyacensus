...
#' @return Numeric growth rate as percentage (invisibly)
#' @export
#' @examples
...
calculate_growth_rate <- function(year1, year2) {
  ...
  message(sprintf("Growth rate %d-%d: %.2f%% per year", year1, year2, rate))
...
#' @return Character vector of matching county names (invisibly)
#' @export
#' @examples
#' search_counties("Nai")
#' search_counties("Kis")
search_counties <- function(pattern, ignore_case = TRUE) {
  ...
  if (length(matches) == 0) {
    ...
    message("Found ", length(matches), " match(es): ", paste(matches, collapse = ", "))
  }
